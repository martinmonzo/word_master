"""
TODO:
    1. Devolver subfrases
    2. Guardar palabras como "altamar"
"""
...
def obtener_acepcion(definicion_html):
    ...
    # No todos los abbrs con class="d" deben ser eliminados de la definición
...
def tiene_otra_grafia(elemento_padre):
    ...
    if (
        ...
        # Verificar que el "También" no pertenezca a una frase
        hijos = elemento_padre.find_all(recursive=False)
        k5_6 = elemento_padre.find_all('p', {'class': ['k5', 'k6']})
        for hijo in hijos:
            if hijo == n1:
                elemento_hijo = n1.find('a', {'class': ['av', 'af']})
                if elemento_hijo and _es_monopalabra(elemento_hijo.text.replace(',', '').replace('.', '')):
                    return True
            elif hijo in k5_6:
                return False
    return False
...
def obtener_otra_grafia(elemento_padre, palabra_original, definicion_original):
    ...
    palabra_otra_grafia = elemento.find('a', {'class': ['av', 'af']}).text.replace(',', '').replace('.', '')

    palabra_original_dividida = palabra_original.split(',')
    palabra_original_dividida[0] = palabra_otra_grafia
    
    definicion['palabra'] = ','.join(palabra_original_dividida)
...
def obtener_definiciones_de_frases(soup):
    ...
    for elemento in elementos:
        if elemento.get('class')[0] in ['k5', 'k6']:
            frase = elemento.text
            frases_y_definiciones[frase] = []
        elif elemento.get('class')[0] == 'm':
            frases_y_definiciones[frase].append(elemento)
